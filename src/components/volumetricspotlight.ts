...
function createConeGeometry(halfAngle: number, distance: number): CylinderGeometry {
...
export function createVolumetricSpotLight(camera: Camera, scene: Scene): UpdateFunc {
  ...
  function onMouseMove(event: MouseEvent): void {
    ...
    const mousePosition = new Vector3(mouseX, mouseY, 0.5);
    mousePosition.unproject(camera);

    // set spotlight target to mouse position
    spotLightTarget.position.copy(mousePosition);

    // rotate cone to look at target
    rotateCone();
  }
